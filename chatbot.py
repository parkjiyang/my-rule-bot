...
# ✨ 재사용을 위한 다운로드 버튼 생성 함수 (key_prefix 추가)
def display_download_buttons(filenames, key_prefix):
    for filename in filenames:
        ...
        try:
            ...
            st.download_button(
                ...
                key=f"{key_prefix}_{filename}" # ✨ 고유 key 추가
            )
...
with st.sidebar:
    ...
    with st.expander("규정집 원본 파일 다운로드", expanded=False):
        for cat_id, cat_info in CATEGORIES.items():
            if cat_info.get('pdf_files'):
                st.subheader(f"📂 {cat_info['name']}")
                # ✨ 사이드바용 고유 key_prefix 전달
                display_download_buttons(cat_info['pdf_files'], key_prefix=f"sidebar_{cat_id}")
                st.markdown("---")
...
if st.session_state.category is None:
    ...
    if st.session_state.category != "global":
        pdf_files_for_category = CATEGORIES[st.session_state.category].get('pdf_files', [])
        if pdf_files_for_category:
            with st.expander(f"'{category_name}' 관련 규정 원본 다운로드"):
                # ✨ 채팅창용 고유 key_prefix 전달
                display_download_buttons(pdf_files_for_category, key_prefix=f"main_{st.session_state.category}")
